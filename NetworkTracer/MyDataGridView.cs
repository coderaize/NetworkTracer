...
namespace NetworkTracer
{
    public class MyDataGridView : DataGridView
    {
        ...
        private bool _IsSmallFont { get; set; } = true;
        public bool IsSmallFont
        {
            get => _IsSmallFont;
            set { UpdateViewDesign(); _IsSmallFont = value; }
        }
        ...
        public MyDataGridView()
        {
            ...
            if (!this.IsInDesignMode())
            {
                ...
                // Adding A Row if last row cell value is changed

                //CellValueChanged += (x, y) =>
                //{
                //    if (AllowUserToAddRows)
                //        if (y.ColumnIndex != -1
                //            && y.RowIndex == Rows.Count - 1
                //            && !Rows[y.RowIndex].Cells[y.ColumnIndex].Value.Equals(null))
                //        {
                //            Invoke(new Action(delegate
                //            {   
                //                CommitEdit(DataGridViewDataErrorContexts.Commit);
                //                Rows.Insert(y.RowIndex + 1, new DataGridViewRow());
                //            }));
                //        }
                //};
        ...
        private DataGridViewCellStyle _tempdataGridViewRowCellStyle { get; set; }
        private DataGridViewCellStyle _tempdataGridViewColumnCellStyle { get; set; }
